...
**Project Manager**
Operational coordinator for plan execution — maintains execution state by reading `signals.jsonl` per task for stage derivation, owns the background usage monitor (`scripts/usage-monitor.sh watch` via the Monitor tool) and forwards only actionable usage events (critical-stop / restart) to Lead with context, tracks per-task budget data, and at startup emits the per-project Ultra Claude Dashboard deep-link only when the project is connected to the dashboard. Spawned once per plan execution; event-driven, waking on messages from Lead/executors and on its monitor's actionable emits. Produces comprehensive operational reports analyzing token efficiency, budget utilization, communication channel health, repeated work, and system improvement recommendations.

**Usage monitor (`scripts/usage-monitor.sh`)**
The single source of truth for rate-limit usage — one shipped script with a `watch` subcommand (PM runs it persistently via the Monitor tool; emits only on the critical-stop crossing and the work-can-restart reset, including a time-authoritative reset when the known reset time passes while usage data is stale) and a `status` subcommand (one-shot JSON of both windows + a clear/soft/critical band). It resolves the account in one place so no caller hand-reads `usage-status.json`; it also quietly traces >10-min task silence as `silence_observed` events in `events.json` — post-mortem data only, never an alert. There is no separate watchdog agent and no stall alerting: PM owns the monitor, the soft limit is enforced by the Lead's pre-spawn `status` check (never an interrupt), and in push-through mode usage emits are suppressed entirely.
