...
**Code Reviewer**
Writes `take.md` with a standards-aware `REVIEWER TAKE` using `CommunicateTeamMember` immediately after spawn (before plan.md is written), then reviews completed code against standards, architecture, and patterns as a read-only quality gate. Uses `WaitForTeamMember` to receive exit requests and `CommunicateTeamMember` for all verdicts (PASS/FAIL with signal backup). Produces persistent take and feedback files alongside pass/fail verdicts.
