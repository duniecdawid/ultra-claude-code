...
**Watchdog**
Lightweight Haiku-based sensor backed by a bash monitoring script that checks every 60 seconds — two independent rate-limit windows with three tiers each (5h: 80% CONSERVE / 90% PAUSE / 95% KILL, 7d: 90% / 95% / 98%) and executor staleness (>10 min silence, checked against both `events.json` and `signals.jsonl` per task). Uses Monitor tool instead of CronCreate: bash outputs alert lines only on state transitions, so the model wakes only on alerts — zero tokens on clean ticks; reset detection is time-authoritative, emitting USAGE-RESET when a window's known reset time passes even while usage data is stale (no API calls happen during a pause), so a paused plan reliably auto-resumes. Spawned once per plan execution alongside PM; PM validates and forwards to Lead.
