...
Knowledge base for building harness components — skills, agents, hooks, protocols. Use when creating or refactoring a skill or agent, writing descriptions or prompts, auditing session context cost (`scripts/context_audit.py`), or optimising resident text. Knowledge, not process: non-negotiables (Opus floor, engines-by-invocation, description budgets, before/after refactor testing), single-topic references including a measured structural-duplication catalogue, and a mandatory two-stage review gate (structural findings first with user confirmation, then an engine compression pass consumed item by item) via `uc:caveman-reviewer`, batched when a change set settles.
...
**Caveman Reviewer**
Two-stage reviewer of persistent harness text (descriptions, agent prompts, protocol formats, CLAUDE.md sections) — proposition-only, never edits: stage 1 checks the artifact against the structural-optimization catalogue (duplication, altitude, form, payload zones, cross-file Grep), stage 2 invokes the caveman-compress CLI on a scratch copy and returns every cut tagged clean / fixable-with-repaired-wording / harmful, while descriptions use first-party `description-writing.md` rules because the engine preserves frontmatter verbatim. Spawned by `/uc:harness-builder`'s mandatory gate — batched when the change set settles, one spawn per artifact per stage. Parent consumes results item by item with no yield threshold (description changes additionally require the before/after trigger test).
