...
Two sub-flows picked at the start: (a) walks the filesystem for `.claude/ultra/sessions/*.json` across every project, cross-references each session id against Claude Code's own transcript jsonl for real liveness, and removes stale files whose transcript has been silent past the threshold; (b) on Linux/systemd, installs or updates a user-level systemd timer that hourly kills tmux sessions detached from any client for more than 24 hours, solving the "VSCode Remote SSH closes but tmux keeps piling up on the VM" problem without touching attached sessions. Use when session files accumulate after crashes, when disk cleanup is needed, when tmux sessions are piling up from closed VSCode windows, or to clear stuck-active sessions left behind by abruptly-killed terminals. Produces a per-project kill report for sub-flow A and installs/verifies the reaper timer for sub-flow B.
...
**Project Manager**
Event-driven operational coordinator for plan execution — maintains dashboard state, validates watchdog alerts and forwards to Lead with context, and tracks per-task budget data (usage % at start/end of each task). Spawned once per plan execution with no cron of its own; wakes only on messages from Lead, the Haiku watchdog, and executors. Produces comprehensive operational reports analyzing token efficiency, budget utilization, repeated work, and system improvement recommendations.

**Watchdog**
Lightweight Haiku-based sensor that ticks every minute checking usage thresholds (75% soft, 90% hard) and executor staleness (>10 min silence). Spawned once per plan execution alongside PM; always on, costs negligible tokens (~$0.03/5h). Signals PM only when something needs attention — silent when healthy; PM validates signals and forwards to Lead.
