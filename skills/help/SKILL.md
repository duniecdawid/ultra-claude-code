...
On Linux/systemd, installs or updates a user-level systemd timer that hourly kills tmux sessions detached from any client for more than 24 hours, using tmux's `session_last_attached`/`session_created` format vars to compute disconnect age without ever touching attached sessions. Use when tmux sessions are piling up on a remote machine after VSCode Remote SSH windows close, or to verify/reinstall/remove an existing reaper install. Produces a detected-state report and on confirmation installs the reaper script + systemd service + hourly timer under `$HOME`, enables linger, and verifies the timer is active.
