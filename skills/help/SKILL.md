...
**Code Reviewer**
Writes `take.md` with a standards-aware `REVIEWER TAKE` and dual-writes the `REVIEWER_TAKE_READY` signal immediately after spawn (before plan.md is written), then reviews completed code against standards, architecture, and patterns as a read-only quality gate. On FAIL, writes `review-feedback.md` and appends `REVIEW_FAIL` signal; on PASS, appends `REVIEW_PASS` signal. Spawned in parallel with the Executor at task start; formal code review fires when Executor signals "ready for review". Produces persistent take and feedback files alongside pass/fail verdicts.
...
**Project Manager**
Event-driven operational coordinator for plan execution — maintains dashboard state by reading `signals.jsonl` per task for stage tracking (replaces STAGE-DONE/RETRY messages from executors), validates watchdog alerts on both rate-limit windows (5h and 7d) and forwards to Lead with window-qualified context, and tracks per-task budget data (usage % at start/end of each task). Spawned once per plan execution with no cron of its own; wakes only on messages from Lead and the Haiku watchdog. Produces comprehensive operational reports analyzing token efficiency, budget utilization, repeated work, and system improvement recommendations.
...
**Task Executor**
Coordinates per-task execution: runs the task-team startup protocol (reads task.md / signals.jsonl / plan.md / impl.md / shared/lead.md / README as first action), receives the Reviewer's upfront `REVIEWER TAKE` (via SendMessage or by polling signals.jsonl for `REVIEWER_TAKE_READY` + reading `take.md`), writes a thin `plan.md` execution delta, runs a deviation self-check, implements code, and drives parallel review/test cycles via the dual-write signal protocol (signal append + SendMessage best-effort) with timer-based polling (~60s) during wait states. Spawned as the hub of each task team; brokers external docs via `QUERY:` to Lead and judgment calls via `ADVICE REQUEST`. Produces `plan.md` (execution delta) and `impl.md` (implementation delta with INTEGRATION and GOTCHA notes).

**Task Tester**
Runs the task-team startup protocol on lazy-spawn, builds a test strategy against `task.md`'s success criteria and product docs (never `impl.md` — impl.md is for the file list only), then verifies code by running tests, writing missing coverage, and launching frontend in a browser to visually confirm UI works. On FAIL, writes `test-feedback.md` and appends `TEST_FAIL` signal; on PASS, appends `TEST_PASS` signal. Spawned by Lead the moment the Executor signals `code complete` — before impl.md is written, so the startup read runs in parallel with the Executor finishing notes. Produces persistent feedback files alongside pass/fail verdicts with evidence.
