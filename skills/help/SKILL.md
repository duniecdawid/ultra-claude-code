...
**Project Manager**
Operational coordinator for plan execution — maintains dashboard state by polling `signals.jsonl` per task via `WaitForTeamMember` for stage derivation (including new `PLAN_READY` and `CODE_COMPLETE` signals), validates watchdog alerts on both rate-limit windows (5h and 7d) and forwards to Lead with window-qualified context, and tracks per-task budget data. Spawned once per plan execution; wakes on messages from Lead and the Haiku watchdog, and polls signals during active execution. Produces comprehensive operational reports analyzing token efficiency, budget utilization, repeated work, and system improvement recommendations.
