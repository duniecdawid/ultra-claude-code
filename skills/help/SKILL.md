...
**Code Reviewer**
Writes `take.md` with a standards-aware `REVIEWER TAKE` using `CommunicateTeamMember` immediately after spawn (before plan.md is written), then reviews completed code against standards, architecture, and patterns as a read-only quality gate. Uses `WaitForTeamMember` (one persistent inbox monitor that follows signals.jsonl and wakes it on any relevant signal) to receive review/re-review/exit requests and `CommunicateTeamMember` for all verdicts (PASS/FAIL with signal backup). Produces persistent take and feedback files alongside pass/fail verdicts.
...
**Project Manager**
Operational coordinator for plan execution — maintains execution state by reading `signals.jsonl` per task for stage derivation, owns the background usage monitor (`scripts/usage-monitor.sh watch` via the Monitor tool), forwards only actionable usage events (critical-stop / restart) to Lead with context, verifies the monitor's NUDGE liveness candidates (a task silent with no named `WAITING_ON` wait and no repo activity — PM verifies, pings the executor, and escalates to Lead only on a confirmed non-response), tracks per-task budget data, and at startup emits the per-project Ultra Claude Dashboard deep-link only when the project is connected to the dashboard. Spawned once per plan execution; event-driven, waking on messages from Lead/executors and on its monitor's actionable emits. Produces comprehensive operational reports analyzing token efficiency, budget utilization, communication channel health, repeated work, and system improvement recommendations.

**Usage monitor (`scripts/usage-monitor.sh`)**
The single source of truth for rate-limit usage — one shipped script with a `watch` subcommand (PM runs it persistently via the Monitor tool; emits only on the critical-stop crossing, the work-can-restart reset including a time-authoritative reset when the known reset time passes while usage data is stale, and `NUDGE` liveness candidates per the protocol's yield rule: task silent >10 min with no named `WAITING_ON` wait and no repo file activity) and a `status` subcommand (one-shot JSON of both windows + a clear/soft/critical band). It resolves the account in one place so no caller hand-reads `usage-status.json`; it also quietly traces >10-min task silence (`silence_observed`) and mid-execution window rollovers (`usage_window_rolled`) into `events.json` — post-mortem data only, never alerts. There is no separate watchdog agent and no blanket stall alerting: silence alone is never escalated, PM verifies every NUDGE candidate before pinging, the soft limit is enforced by the Lead's pre-spawn `status` check (never an interrupt), and in push-through mode usage emits are suppressed entirely (NUDGE still fires — liveness, not usage).
