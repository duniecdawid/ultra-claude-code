...
**Watchdog**
Lightweight Haiku-based sensor that ticks every minute checking two independent rate-limit windows with three tiers each (5h: 80% CONSERVE / 90% PAUSE / 95% KILL, 7d: 90% / 95% / 98%) and executor staleness (>10 min silence). Spawned once per plan execution alongside PM; always on, costs negligible tokens (~$0.03/5h). Signals PM with window-qualified alerts (CONSERVE/PAUSE/KILL/USAGE-RESET) only on transitions — silent when healthy; PM validates and forwards to Lead.
