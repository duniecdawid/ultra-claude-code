...
**Watchdog**
Lightweight Haiku-based sensor that ticks every minute checking two independent rate-limit windows (5h: 80% soft / 90% hard, 7d: 90% soft / 95% hard) and executor staleness (>10 min silence). Spawned once per plan execution alongside PM; always on, costs negligible tokens (~$0.03/5h). Signals PM with window-qualified alerts only when something needs attention — silent when healthy; PM validates signals and forwards to Lead.
