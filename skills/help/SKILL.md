...
Connects to the Ultra Claude Dashboard at `dashboard.ultra-claude.dev` for real-time project visibility, and manages multiple dashboard accounts with optional per-project account routing. Use when setting up dashboard sync, troubleshooting agent connectivity, checking sync status, or controlling which account a project syncs to. Guides agent installation and setup, documents the non-interactive CLI account commands (`assign`/`default`/`auto-assign`) plus the interactive REPL — including the rule to force a one-shot `push <project> --account <account>` after any account switch so the new account is back-filled — and verifies connectivity with self-contained debug checks.

**Setup** (`/uc:setup`)
One-time machine configuration that installs prerequisites (Node.js, optionally tmux), guides tmux mode selection (per-project, per-terminal, none, or custom), configures shell environment (1M context, agent teams), offers the Claude Code fullscreen renderer for flicker-free flat-memory output (opt-out, gated on Claude Code v2.1.89+, written as `"tui": "fullscreen"` in settings.json), offers to update the `ultraclaude-agent` npm package when already installed and outdated (never auto-installs), checks dashboard connectivity when the agent is installed, (optionally) scaffolds a user-level `machine-context` skill at `~/.claude/skills/machine-context/` via an interactive interview capturing the user's Chrome setup, VM/host topology, dev runtimes, and network conventions, (when VS Code is detected, opt-in) offers recommended client-side VS Code settings for the Claude Code extension printed as merged JSON to paste — alongside the mode-aware terminal profiles it already writes per tmux mode, and (for per-terminal mode only, Linux/systemd, strictly opt-in) installs a user-level tmux disconnected-session reaper that hourly kills tmux sessions detached from any client for more than 24 hours. Use after plugin installation, on a new machine, when fixing screen tearing/flicker, to add machine-context after the fact, to change tmux mode, to configure VS Code for Claude Code, or to pick up a newly-published `ultraclaude-agent` version. Idempotent — writes a version marker, never clobbers user-written machine-context files without explicit confirmation.
...
Cache-first external research covering library/API documentation, architectural patterns, and market/competitor analysis via a single auto-classified interface — fresh cache hits read directly from disk while cache misses spawn the stateless `researcher` subagent. Use when adding libraries, investigating patterns or best practices, running competitor analysis, asking how Claude Code / Claude Agent SDK / Anthropic API features work, or any "how does X work / what changed in X / best practice for X" question — Claude/Anthropic topics auto-classify as `library` mode and the researcher queries Ref.tools plus the curated `docs.claude.com` URL roots in parallel. Produces committed research files in two scopes with separate indexes: product/domain research under `documentation/technology/research/{libraries,patterns}/` and `documentation/product/research/` (indexed at `documentation/technology/research/index.json`), and Claude harness research under `.claude/ultra/research/` (flat layout, indexed at `.claude/ultra/research/index.json`) — frontmatter-driven per-entry staleness (library 10d, patterns 90d, market 30d, historical research frozen).
...
**Code Reviewer**
Writes `take.md` with a standards-aware `REVIEWER TAKE` using `CommunicateTeamMember` immediately after spawn (before plan.md is written), then reviews completed code against standards, architecture, and patterns as a read-only quality gate. Uses `WaitForTeamMember` (bounded Monitor rounds that keep it responsive to SendMessage) to receive review/re-review/exit requests and `CommunicateTeamMember` for all verdicts (PASS/FAIL with signal backup). Produces persistent take and feedback files alongside pass/fail verdicts.
...
**Project Manager**
Operational coordinator for plan execution — maintains execution state by polling `signals.jsonl` per task via `WaitForTeamMember` for stage derivation (including new `PLAN_READY` and `CODE_COMPLETE` signals), validates watchdog alerts on both rate-limit windows (5h and 7d) and forwards to Lead with window-qualified context, tracks per-task budget data, and at startup emits the per-project Ultra Claude Dashboard deep-link to Lead only when the project is connected to the dashboard. Spawned once per plan execution; wakes on messages from Lead and the Haiku watchdog, and polls signals during active execution. Produces comprehensive operational reports analyzing token efficiency, budget utilization, repeated work, and system improvement recommendations.

**Watchdog**
Lightweight Haiku-based sensor backed by a bash monitoring script that checks every 60 seconds — two independent rate-limit windows with three tiers each (5h: 80% CONSERVE / 90% PAUSE / 95% KILL, 7d: 90% / 95% / 98%) and executor staleness (>10 min silence, checked against both `events.json` and `signals.jsonl` per task). Uses Monitor tool instead of CronCreate: bash outputs alert lines only on state transitions, so the model wakes only on alerts — zero tokens on clean ticks. Spawned once per plan execution alongside PM; PM validates and forwards to Lead.
