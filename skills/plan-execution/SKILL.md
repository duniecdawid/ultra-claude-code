...
| PM: `"STALL: task-{N} ..."` | **The sole stall net** now that agents run one persistent inbox monitor with no self-timeout (protocol §3). First rule out **benign** stalls (no action, not a failure): a pipeline-parked successor in your `shared/lead.md` parked list whose predecessor is still in flight is *expected* to be silent — it will get `IMPL_APPROVED` when the predecessor passes; likewise a task legitimately parked on a slow-but-alive counterparty just needs the counterparty to finish. Otherwise investigate the named counterparty: alive? stuck? crashed? Re-send/re-signal the missing item via `CommunicateTeamMember`, or apply Phase 4 failure handling (re-spawn) if it's dead. If you cannot resolve it, record a **visible hard-fail**: `echo '{"ts":"...","signal":"WAIT_TIMEOUT","author":"lead","note":"unresolved STALL: {SIGNAL} from {agent} in task-{N}"}' >> tasks/task-{N}/signals.jsonl`, then escalate or re-spawn. Never let a parked agent hang silently. |
