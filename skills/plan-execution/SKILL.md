...
- **Signal protocol**: per-task `signals.jsonl` provides durable pipeline state as a fallback for unreliable SendMessage delivery to executors. All stage transitions are dual-written (content file → signal append → SendMessage best-effort). PM reads signals.jsonl for dashboard state derivation. See `${CLAUDE_PLUGIN_ROOT}/skills/plan-execution/references/signal-protocol.md`.
...
| **Signal protocol** | All agents → `signals.jsonl` | Per-task JSONL append-only file for durable pipeline state. Dual-write: content file (if any) → `echo >> signals.jsonl` → SendMessage (best-effort). PM reads signals.jsonl for stage derivation (replaces STAGE-DONE/RETRY messages). See `signal-protocol.md`. |
