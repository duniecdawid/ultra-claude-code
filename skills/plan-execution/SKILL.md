...
- **Execution communication protocol**: all inter-agent communication uses three functions — `CommunicateTeamMember` (direct), `CommunicateTeam` (broadcast), and `WaitForTeamMember` (receive). Each wraps SendMessage with a durable file-based signal layer (per-task `signals.jsonl`). See `${CLAUDE_PLUGIN_ROOT}/skills/plan-execution/references/execution-communication-protocol.md`.
...
| **Communication protocol** | All agents → `signals.jsonl` + SendMessage | Unified protocol with three functions: `CommunicateTeamMember` (direct), `CommunicateTeam` (broadcast), `WaitForTeamMember` (receive). Each wraps SendMessage with a durable signal layer. See `execution-communication-protocol.md`. |
