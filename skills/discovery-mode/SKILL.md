...
**1. Explore subagent** (spawn via the `Agent` tool, one-shot fan-out: no `name`, explicit `run_in_background: true` — Mode F per `${CLAUDE_PLUGIN_ROOT}/references/agent-spawn-modes.md`; stream 2's market researcher also dispatches in the background on a cache miss, so both streams run concurrently — collect **every** completion notification before Phase 3 synthesis) — internal codebase and technical research:
