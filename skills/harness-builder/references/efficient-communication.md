...
3. **Build-workflow compression** — spawn the `uc:caveman-compress` agent (this plugin) on body artifacts at stage 4 of the build workflow; it runs the engine on a scratch copy and returns a classified cut list, consumed item by item. **Not optional** for build tasks: spawn form and adoption rules are in `stage-4-compression.md`.
...
6. **Model:** with `ANTHROPIC_API_KEY` set it uses the SDK and defaults to `claude-sonnet-4-5` — below the Opus floor (non-negotiable #1); export `CAVEMAN_MODEL=claude-opus-5`. Without the key it shells out to `claude --print`, which runs the machine's configured default model — and `CAVEMAN_MODEL` is then a **no-op**, since only the SDK path reads it (`compress.py:142` vs the shell-out at `:158`) [MEASURED 2026-07-30]. On that path the floor rests entirely on the machine default: check it (`~/.claude/settings.json`) rather than trusting the env var.
