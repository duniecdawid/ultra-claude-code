# Plan: {Name}

> **Execute:** `/uc:plan-execution {NNN}`  <!-- NNN = 3-digit zero-padded number, e.g. 001 -->
> Created: {date}
> Status: Stub | Draft | Approved | In Progress | Completed | Cancelled
> Source: Feature Mode | Debug Mode | Verification Mode | Migration

## Objective

What this plan accomplishes. One or two sentences.

## Context

References to the documentation that informs this plan. Prefer links over prose — if the information lives in a doc, link to it rather than restating it here.

- **Architecture:** [link to relevant architecture doc(s)]
- **Product:** [link to product description in documentation/product/description/]
- **Requirements:** [link to relevant requirements doc(s)]
- **RFCs:** [link to relevant RFC(s), if any]
- **Other:** [any additional context references]

## Tech Stack

Narrative summary of the external libraries, frameworks, and services this plan depends on. For the authoritative per-task research mapping, see each task's `tasks/task-N/task.md` `**Research:**` section — that's where pointers to the durable research files live.

- {technology 1}
- {technology 2}

## Scope

### In Scope

-

### Out of Scope

-

## Success Criteria

- [ ]

## Task Sizing

Final sizing table from the Stage 4 task sizing gate (see `references/planning-framework/task-sizing.md`). For a single-task plan, replace the table with: `1 task ({type}, {model}) — no split needed.`

| Task | Type | Model | Est. files | Why it can't merge with a neighbor | Verdict |
|------|------|-------|-----------|-----------------------------------|---------|
| 1. {Title} | code/ops | sonnet/opus/fable | ~N | {independence justification} | in band / justified exception |

## Task List

Flat index only. Per-task details (description, files, patterns, success criteria, research pointers, dependencies) live in `tasks/task-N/task.md`.

### Task 1: {Title} <!-- status:pending -->
- [ ] **Complete**

### Task 2: {Title} <!-- status:pending -->
- [ ] **Complete**

## Documentation Changes

Documentation updated during Stage 4 (already on disk):

| File | Action | Summary |
|------|--------|---------|
| `documentation/technology/architecture/{file}.md` | Created / Updated | One-sentence description of what was added or changed |

Additional documentation gaps identified (not yet addressed):

| File | Needed Change |
|------|---------------|
| | |

## Risk Assessment

| Risk | Likelihood | Impact | Mitigation |
|------|-----------|--------|------------|
| | | | |
