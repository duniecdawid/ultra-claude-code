...
- **On any `mcp__claude-in-chrome__*` failure, invoke `/chrome-debug` as your first recourse** — do NOT immediately fall back to code-level verification. The `chrome-debug` skill covers the most common failure modes (stale native host, suspended service worker, bridge race conditions) and can auto-recover without user intervention in most cases. It's designed to run autonomously in automated test cycles and will only escalate when genuine user action is needed (reload extension, click Connect). This is a hard requirement, not a suggestion — plan 003 and plan 009 both saw testers give up after 3–4 browser failures when a 30-second `/chrome-debug` run would have recovered the connection.
- **Escalation path if `/chrome-debug` fails:** relay the specific diagnostic output to the PM agent (if one is running) OR directly to the user, telling them exactly what physical action is needed (e.g., "click the Claude in Chrome extension icon on the Mac once to wake the service worker"). Wait for a reasonable acknowledgment.
- **Only after `/chrome-debug` has been run AND escalation has been attempted** should you fall back to code-level verification. When you do fall back, note in your report both that browser testing was unavailable AND that `/chrome-debug` was invoked and what its diagnostic said — this gives the Lead visibility into recurring environmental issues rather than silently accepting code-only verification.
