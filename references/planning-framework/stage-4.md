...
- **All research is collected — no pending dispatches.** Every researcher the Stage 2 sweep (or any later `/uc:research` call) dispatched in the background must have delivered its completion notification. If any dispatch is still pending, wait for its notification before writing anything — Stage 4 bakes research pointers and library facts into task.md files, and a half-written research file poisons every task that points at it. Backstop check (notifications can be missed): for each `target_path` returned during Stage 2, the file must exist on disk and its index entry must not be stale — verify mechanically with the freshness jq from the research skill, or a simple existence + `fetched_at` check. A dispatch that reported `status: "failed"` doesn't block, but its gap must be named in the plan's Risk Assessment.

This check exists because the most common failure mode is jumping from a detailed user request straight to writing a plan. A detailed request makes Stages 1-3 faster, not unnecessary — fast research still catches things the user missed, and fast discussion still validates your approach. The research-collection gate exists because background dispatches make it possible to reach Stage 4 while researchers are still writing — Stage 4 must never race them.
